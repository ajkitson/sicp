...
; as a list of bindings, where each binding is a name-value pair. Reqrite the
; environment operations to use this alternative representation.
; ============
; We'll have to change how frames are constructed, traversed, and manipulated,
; but can leave how environments are constructed alone (as long as we don't
; change the api on make-frame)

; First, we need to zip together the variables and values. We can just define
; make-frame as the built-in zip (which will make the bindings lists, not pairs,
; which is OK):
  ...
  (define (env-loop env)
    (define (scan bindings)
      (cond
        ...
        ((eq? var (first-variable bindings)) (first-value bindings))
  ...
  (define (env-loop env)
    (define (scan bindings)
      (cond
        ((null? bindings)
          (env-loop (enclosing-environment env)))
        ((eq? var (first-variable bindings)) (set-car! bindings (list var val)))
        (else (scan (cdr bindings)))))
    (if (eq? env the-empty-environment)
...
; ============
; Below is the normal evaluator with pieces commented out
; ============
