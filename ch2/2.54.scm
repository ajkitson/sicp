...
; if they are both symbols and the symbols are eq?, or if they are both lists such that (car a) is equal? to (car b) and (cdr a) is
; equal? to (cdr b). Using this idea, implement equal? as a procedure.
; =====
; Here's we go:
(define (equal? a b)
	(cond 
		((and (not (pair? a)) (not (pair? b)))	; both symbols, the base case
			(eq? a b))
		((and (pair? a) (pair? b))				; both lists
			(and (equal? (car a) (car b))
				(equal? (cdr a) (cdr b))))
		(else false)))							; mismatch: one symbol, one list
